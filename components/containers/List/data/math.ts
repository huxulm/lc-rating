    ...
    "nonLeafChild": [
        ...
        },
        {
            ...
            "nonLeafChild": [
                ...
                {
                    "title": "§1.5 因子",
                    "summary": "模板（预处理每个数的所有因子）：<br>```py [sol-Python3]<br>MX = 1_000_001<br>divisors = [[] for _ in range(MX)]<br>for i in range(1, MX):<br>for j in range(i, MX, i):<br>divisors[j].append(i)<br>```<br>```java [sol-Java]<br>class Solution {<br>private static final int MX = 1_000_001;<br>private static final List<Integer>[] divisors = new ArrayList[MX];<br>private static boolean initialized = false;<br>// 这样写比 static block 更快<br>private void init() {<br>if (initialized) {<br>return;<br>}<br>initialized = true;<br>Arrays.setAll(divisors, _ -> new ArrayList<>());<br>for (int i = 1; i < MX; i++) {<br>for (int j = i; j < MX; j += i) {<br>divisors[j].add(i);<br>}<br>}<br>}<br>public int solve(int[] nums) {<br>init(); // 记得初始化<br>}<br>}<br>```<br>```cpp [sol-C++]<br>const int MX = 1'000'001;<br>vector<int> divisors[MX];<br>int init = [] {<br>for (int i = 1; i < MX; i++) {<br>for (int j = i; j < MX; j += i) {<br>divisors[j].push_back(i);<br>}<br>}<br>return 0;<br>}();<br>```<br>```go [sol-Go]<br>const mx = 1_000_001<br>var divisors [mx][]int // 如果 mx 很大可能会 MLE，可以改成 int32<br>func init() {<br>for i := 1; i < mx; i++ {<br>for j := i; j < mx; j += i {<br>divisors[j] = append(divisors[j], i)<br>}<br>}<br>}<br>```<br>",
                    "src": "",
                    "original_src": "",
                    "sort": 0,
                    "isLeaf": false,
